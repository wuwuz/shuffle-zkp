...
type attributionCircuit struct {
	// public statements
...
func (circuit *attributionCircuit) Define(api frontend.API) error {
...
func (c *ClientState) GenAssignment(setX *SetAccumulator, setY *SetAccumulator) attributionCircuit {
	...
	if !setX.VerifyMerkleProof(c.CommSrnX[0], merkleProofX[:MerkleTreeHeight], merklePositionX[:MerkleTreeHeight]) {
		...
		//log.Printf("the first merkle proof is correct\n")
	}
...
func GenProofGroth16(assignment attributionCircuit, ccs *constraint.ConstraintSystem, pk *groth16.ProvingKey) (*groth16.Proof, *witness.Witness) {
	// witness definition
...
func GenProofPlonk(assignment attributionCircuit, ccs *constraint.ConstraintSystem, pk *plonk.ProvingKey) (*plonk.Proof, *witness.Witness) {
	// witness definition
	witness, _ := frontend.NewWitness(&assignment, ecc.BN254.ScalarField())
	//fmt.Println(witness)
	publicWitness, _ := witness.Public()
...
func AttributionGroth16() {
	...
	// define a dummy circuit for attribution
	var circuit = attributionCircuit{
		PublicCommitment: 0,
		MerkleRootX: 0,
		MerkleRootY: 0,
	...
	start = time.Now()
	allAssignment := make([]attributionCircuit, ClientNum)
	for i := 0; i < len(clients); i++ {
	...
	// finally, the server checks for repeated serial numbers
	start = time.Now()
...
func main() {
	...
	/*
	var err error
	file, err = os.OpenFile("output-attribution.csv", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		panic(err)
	}
